...
export function formatPipelineOffer(offer) {
  ...
  const base = `- [ ] ${url} | ${company} | ${title}${fit}${postedDateUnknown}`;
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        if (cooldownResult.skip) {
          ...
          cooldownOffers.push({
            job: { ...scoredJob, source: sourceName },
  ...
  if (verifiedOffers.length > 0) {
    ...
    for (const o of verifiedOffers) {
      const trustSuffix = o.trustScore != null && o.trustScore < 100
        ...
        : '';
      const fitSuffix = Number.isFinite(o.fitScore) ? ` | ${o.fitScore} (${o.fitBand})` : '';
      console.log(`  + ${o.company} | ${o.title} | ${o.location || 'N/A'}${fitSuffix}${trustSuffix}`);
    }
    if (dryRun) {
